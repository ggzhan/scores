...
class Swa {
    ...
    func loadAudioSignal(audioURL: URL) -> (signal: UnsafeMutablePointer<Float>, length: Int) {
        ...
        let array = UnsafeMutablePointer<Float>.allocate(capacity: floatArray.count)
        for i in 0...floatArray.count-1 {
            array[i] = floatArray[i]
        }
        return (array, floatArray.count)
    }
    
    func getSoundFile(soundFileURL: URL) -> (signal: UnsafeMutablePointer<Float>, length: Int) {
        let soundFile = loadAudioSignal(audioURL: soundFileURL).signal
        let length = loadAudioSignal(audioURL: soundFileURL).length
     ...
     */
    func generate_chroma_kernel(N: Int, fmin: Float, fmax: Float, fs: Float) -> UnsafeMutablePointer<UnsafeMutablePointer<Float>> {
        let alpha = powf(2.0, 1/12.0)
        let fLow = (fmin + fmin / alpha) / 2.0
        let fHigh = (fmax + fmax * alpha) / 2.0
        let frequencies = UnsafeMutablePointer<Float>.allocate(capacity: N)
        for i in 0...N-1 {
            frequencies[i] = 0.0
        }
        let margin = Int(fs)/N     //margin= nyquist/(N/2)
        for i in 0...N/2 {        // couldnt find a better way to do this....
            frequencies[i] =  Float(i*margin)
        }
        
        let kernel = UnsafeMutablePointer<UnsafeMutablePointer<Float>>.allocate(capacity: N)   //tested in Playground, should be working correctly
        for j in 0...N/2 {
            if frequencies[j] == 0 {
                continue
            }
            if (fLow <= frequencies[j]) && (frequencies[j] <= fHigh) {
                let pitchClass = frequency_to_midi(f: frequencies[j])%12
                kernel[j][pitchClass] = 1.0
            }
        }
        return kernel
    }
    ...
    public func fourier_chroma(x: UnsafeMutablePointer<Float>, length: Int, fmin: Float=65.406, fmax: Float=20000,fs: Float=44100, weights: FFTSetup?) -> UnsafeMutablePointer<Float> {
        
        let tempChromaKernel = generate_chroma_kernel(N: length, fmin: 65.406, fmax: 20000, fs: 44100)
        let signalEnergy = fft_p(x, length: length, weights: weights)
        let chromaKernel = dot_p(a: tempChromaKernel, b: signalEnergy) //transpose and dot in one
        let constant = sqrt(Float(length))
    ...
    public func extract_features(x: UnsafeMutablePointer<Float>, length: Int, winSize: Int, winOffset: Int) -> UnsafeMutablePointer<UnsafeMutablePointer<Float>> {
        let nrOfBlocks = Int(length/winSize)
        let length_fft = vDSP_Length(floor(log2(Float(length))))
        let radix = FFTRadix(kFFTRadix2)
        let weights = vDSP_create_fftsetup(length_fft, radix)
        //var ret: [[Float]] = Array(repeating: Array(repeating: 0.0, count: n), count: nrOfBlocks)
        let y = UnsafeMutablePointer<Float>.allocate(capacity: winSize)
        let ret = UnsafeMutablePointer<UnsafeMutablePointer<Float>>.allocate(capacity: nrOfBlocks)
        ...
        for i in 0...nrOfBlocks-1 {  //in cases where x.count is multiple of window size, index out of range for nrOfBlocks-1
            for j in 0...winSize-1 {
                y[j] = x[i*winOffset+winSize-1]
            }
            ret[i] = fourier_chroma(x: y, length: length, weights: weights)
        }
        return ret
    }
    
    func getFeatures(recordings: [URL])-> (refFeatures: UnsafeMutablePointer<UnsafeMutablePointer<Float>>, testFeatures: UnsafeMutablePointer<UnsafeMutablePointer<Float>>) {
        let refFeatures = extract_features(x: getSoundFile(soundFileURL: recordings[1]).signal, length: getSoundFile(soundFileURL: recordings[1]).length, winSize: windowSize, winOffset: windowOffset)
        let testFeatures = extract_features(x: getSoundFile(soundFileURL: recordings[0]).signal,length: getSoundFile(soundFileURL: recordings[0]).length, winSize: windowSize, winOffset: windowOffset)
        return (refFeatures, testFeatures)
    }
